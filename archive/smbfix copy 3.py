...
# **List All Incorrect Ownership Folders**
...
# **List All Incorrect Ownership Files**
...
def unlock_file(path):
    ...
    if result.stdout.strip():
        ...
        if logged_in_user not in stored_passwords:
            stored_passwords[logged_in_user] = getpass.getpass(f"Password for {logged_in_user}: ")

        cmd = f'chflags -R nouchg "{path}"'
        child = subprocess.run(["su", "-", logged_in_user, "-c", cmd], input=stored_passwords[logged_in_user], text=True)
